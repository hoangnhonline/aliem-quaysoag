...
function submitUserScore(score, type){
	...
	if(!validatePhone($('#uPhone').val())){
		errorCon = true;
		errorMessage += '\n*Please enter a valite phone number';
	}
	if(errorCon){
		alert(errorMessage);	
	}else{
		...
		$.ajax({
		  ...
		  data: { name: $('#uName').val(), email: $('#uEmail').val(), phone: $('#uPhone').val(), type:type, score:score, enableLevel:enableLevel},
...
function validatePhone($phone) {
  var phoneReg = /(09|01[2|6|8|9])+([0-9]{8})\b/;
